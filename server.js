...
/**
 * Searches Target Domains safely with Cheerio
 */
async function searchTargetDomain(domainConfig, query) {
  try {
    ...
    // CLEANED BLACKLIST: Removed path terms (/movies/, /web-series/) that broke real hits
    const blacklist = [
      "category", "tag", "author", "contact", "about", "how-to-download", "?s=",
      "wp-content", "login", "register", "dmca", "disclaimer", "privacy", "policy",
      "terms", "faq", "comment", "movies-by-year", "movies-by-genre",
      "movies-by-size", "/page/", "request", "credits"
    ];

    $('article, .post, .type-post, .result-item, a').each((_, element) => {
      let href = null;
      let $a = null;
      ...
      if ($(element).is('a')) {
        ...
        $a = $(element).find('h2 a, h3 a, .entry-title a, a').first();
        href = $a.attr('href');
      }
      ...
      if (isInternal && !isBlacklisted && lowerHref.length > 22) {
        const fullUrl = href.startsWith("/") ? `${domainConfig.baseUrl}${href}` : href;
        let anchorTitle = $a.text().replace(/\s+/g, ' ').trim();

        if (anchorTitle.length <= 3) {
          try {
...
/**
 * Scrapes target page for download links and screenshots
 */
async function scrapeTargetPage(targetUrl, cleanTitle) {
  ...
  try {
    ...
    $('h1, h2, h3, h4, p, div, strong, b, a').each((_, el) => {
      ...
      if (tagName !== 'a' && text.length > 4 && text.length < 100 && /480p|720p|1080p|2160p|4k|mb|gb|hindi|english/i.test(text)) {
...
<body>
    ...
    <script>
        ...
        async function executeSearch() {
            ...
            try {
                ...
                if (data.results && data.results.length > 0) {
                    window.searchCache = data.results; // Safe index-based caching
                    data.results.forEach((item, idx) => {
                        const delay = idx * 0.05;
                        let visualElement = item.image 
                            ? '<img src="' + item.image + '" class="poster-img" loading="lazy" alt="Cover">'
                            : '<div class="poster-bg-gradient" style="background: ' + getGradientForTitle(item.title) + '"></div>';
                        
                        grid.innerHTML += 
                            '<div class="poster-card" style="animation-delay: ' + delay + 's" onclick="openDetail(' + idx + ')">' +
                                visualElement +
                                '<div class="poster-overlay"></div>' +
                                '<div class="poster-content">' +
                                    '<div class="poster-title">' + escapeHTML(item.title) + '</div>' +
                                    '<div class="poster-domain">' + escapeHTML(item.domain) + '</div>' +
                                '</div>' +
                            '</div>';
                    });
                } else {
        ...
        async function openDetail(idx) {
            const item = window.searchCache ? window.searchCache[idx] : null;
            if (!item) return;
            ...
            modalBody.innerHTML = 
                '<div class="modal-hero">' +
                    ...
                    '<div class="modal-info">' +
                        ...
                        '<div><span class="tag" style="display:inline-block; margin-bottom: 5px;">Source: ' + escapeHTML(domain) + '</span></div>' +
            ...
            try {
                ...
                if (source.status === "success") {
                        ...
                        source.links.forEach(linkObj => {
                            html += 
                                '<a href="' + linkObj.url + '" target="_blank" class="dl-btn">' +
                                    '<div class="dl-info">' +
                                        '<span class="dl-title">' + escapeHTML(linkObj.title) + '</span>' +
                                        '<span class="dl-url">' + escapeHTML(linkObj.url.substring(0, 50)) + (linkObj.url.length > 50 ? "..." : "") + '</span>' +
                                    '</div>' +
                    ...
                    html += '<div class="status-msg error">' + escapeHTML(source.error || "Scraping routine failed.") + '</div>';
                }
