...
// --- ROUTE 3: Precise Direct-URL Extraction API ---
app.get("/api/scrape", async (req, res) => {
  ...
  try {
    ...
    console.error("Scrape route failure:", err);
...
async function searchTargetDomain(domainConfig, query) {
  try {
    ...
    if (!response.ok) {
      console.log(`[${domainConfig.name}] HTTP Error ${response.status}`);
      return [];
    }

    const rawHtml = await response.text();
    if (rawHtml.includes("Just a moment...") || rawHtml.includes("challenge-platform")) {
      console.log(`[${domainConfig.name}] Blocked by Cloudflare challenge screen.`);
      return [];
    }
    ...
    const blacklistedWords = [
      ...
      "terms", "faq", "comment", "movies-by-year", "movies-by-genre", "/page/", 
      "request", "credits"
    ];
    ...
    if (postCards.length > 0) {
      postCards.each((_, card) => {
        ...
        const isBlacklisted = blacklistedWords.some(word => lowerHref.includes(word));
        ...
        if (isInternal && !isBlacklisted && lowerHref.length > 20) {
          ...
          if (imgEl.length) {
            let src = imgEl.attr("data-src") || imgEl.attr("data-lazy-src") || imgEl.attr("src") || imgEl.attr("data-original");
            if (src && !src.includes("data:image") && !src.includes("avatar")) {
              image = src.startsWith("/") ? `${domainConfig.baseUrl}${src}` : src;
            }
          }
    ...
    console.error(`[${domainConfig.name}] Scrape error:`, error.message);
    return [];
  }
}
...
async function scrapeTargetPage(targetUrl, cleanTitle) {
  ...
  try {
    ...
    $("body *").each((_, el) => {
      if (parserState === "FINISHED") return;

      const $el = $(el);
      const tagName = el.tagName ? el.tagName.toLowerCase() : "";

      if (["p", "div", "h1", "h2", "h3", "h4"].includes(tagName)) {
        const text = $el.clone().children().remove().end().text().trim();
        if (!text) return;

        if (parserState === "SEEKING") {
          const isScreenshotMarker = /screenshot|screen shot/i.test(text);
          const isQualityHeader = text.length > 5 && /480p|720p|1080p|2160p|4k/i.test(text);

          if (isScreenshotMarker || isQualityHeader) {
            parserState = "COLLECTING";
            isScreenshotSection = isScreenshotMarker;
            lastValidHeading = text;
            return;
          }
        }

        if (parserState === "COLLECTING") {
          if (/share with|related movie|you may also like|comment|navigation/i.test(text)) {
            parserState = "FINISHED";
            return;
          }

          if (/screenshot|screen shot/i.test(text)) {
            isScreenshotSection = true;
            lastValidHeading = text;
            return;
          }

          if (text.length > 5 && /480p|720p|1080p|2160p|4k|mb|gb|hindi|english/i.test(text)) {
            if (!["download now", "click here"].some(w => text.toLowerCase().includes(w))) {
              isScreenshotSection = false; 
              lastValidHeading = text;
            }
          }
        }
      ...
      if (tagName === "a") {
        ...
        const lowerHref = href.toLowerCase();
        const isInternal = lowerHref.includes(internalDomain) || href.startsWith("/");
        const isSocialJunk = socialJunk.some(word => lowerHref.includes(word));

        if (!isInternal && !isSocialJunk && lowerHref.startsWith("http")) {
          const looksLikeImage = /\.(jpg|jpeg|png|webp|gif)$/i.test(lowerHref) || lowerHref.includes("postimg") || lowerHref.includes("imgur");

          if (isScreenshotSection || looksLikeImage) {
            if (!extractedScreenshots.includes(href)) {
    ...
    if (extractedLinks.length === 0 && fallbackLinks.length > 0) {
      extractedLinks.push(...fallbackLinks);
    }
...
function getHTML() {
  return [
    ...
    '                }',
    '                ',
    '                document.querySelector(".scraping-indicator").innerHTML = `<span style="color:#94a3b8">Extraction Complete</span>`;',
    '                document.querySelector(".scraping-indicator").style.background = "rgba(255,255,255,0.05)";',
    '                document.querySelector(".scraping-indicator").style.borderColor = "rgba(255,255,255,0.1)";',
    '                document.querySelector(".scraping-indicator").style.color = "#94a3b8";',
    '                ',
    ...
    '                document.querySelector(".scraping-indicator").style.display = "none";',
    '            }',
    '        }',
    '        ',
    '        function closeModal() {',
